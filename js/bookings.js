...
async function loadBookings() {
  try {
    ...
    for (const docSnap of querySnapshot.docs) {
        ...
        // Fetch session details to get the meeting link
        let meetingLink = null;
        if (booking.sessionId) {
            const sessionRef = doc(db, "sessions", booking.sessionId);
            const sessionSnap = await getDoc(sessionRef);
            if (sessionSnap.exists()) {
                meetingLink = sessionSnap.data().meetingLink;
            }
        }
        booking.meetingLink = meetingLink; // Add link to booking object
...
function createBookingCard(booking, isUpcoming) {
    ...
    // Conditionally add the Join Session button
    let actionsHtml = '';
    if (isUpcoming && booking.status === "confirmed") {
        actionsHtml = `
            <div class="booking-actions">
                ${booking.meetingLink ? `<a href="${booking.meetingLink}" target="_blank" class="btn-primary">Join Session</a>` : ''}
